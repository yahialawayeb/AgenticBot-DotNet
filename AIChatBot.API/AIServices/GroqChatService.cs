...
{
    public class GroqChatService : IChatModelService
    {
        private readonly HttpClient _httpClient;
        private readonly ILogger<GroqChatService> _logger;
        private readonly GroqModelsApi _configurations;
        private readonly ToolsRegistryService _toolsRegistryService;
        private readonly IHubContext<ChatHub> _hubContext;

        public GroqChatService(HttpClient httpClient, ILogger<GroqChatService> logger, IOptions<GroqModelsApi> options, ToolsRegistryService toolsRegistryService, IHubContext<ChatHub> hubContext)
        {
        ...
        public async Task<string> SendMessageAsync(string model, string message, string connectionId)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(_configurations.Url))
                    throw new InvalidOperationException("Groq URL is not configured.");
                ...
                {
                    await _hubContext.Clients.Client(connectionId).SendAsync("ReceiveStatus", "🟡 Thinking (Groq)...");
                }
                ...
                return ConvertGroqResponse(stream);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred in GroqChatService.SendMessageAsync");
                throw;
            }
        }
        ...
        public async Task<List<FunctionCallResult>> ChatWithFunctionSupportAsync(string model, List<Dictionary<string, string>> userMessage, string connectionId)
        {
            ...
            {
                _logger.LogError(ex, "Error occurred in GroqChatService.ChatWithFunctionSupportAsync");
                throw;
            }
        ...
        private string ConvertGroqResponse(string modelResponse)
        {
            try
            {
